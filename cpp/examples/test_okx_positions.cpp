...
int main() {
    ...
    // ==================== 订阅持仓频道 ====================
    std::cout << "\n[5] 订阅持仓频道..." << std::endl;
    std::cout << "   ⚠️  重要说明：" << std::endl;
    std::cout << "   - 持仓频道只推送合约和杠杆的持仓（SWAP/FUTURES/OPTION/MARGIN）" << std::endl;
    std::cout << "   - 现货（SPOT）买入后，资产在账户余额中，不在持仓频道推送" << std::endl;
    std::cout << "   - 要查看现货余额变化，请使用账户频道（account）" << std::endl;
    std::cout << "   - 要测试持仓频道，请开合约仓位（如BTC-USDT-SWAP）" << std::endl;
    std::cout << "\n   方式1: 订阅所有类型持仓（定时推送 + 事件推送）" << std::endl;
    ...
    // ==================== 可选：同时订阅账户频道查看现货余额 ====================
    std::cout << "\n[可选] 同时订阅账户频道查看现货余额变化..." << std::endl;
    std::cout << "   💡 提示：现货买入后，余额变化在账户频道中推送" << std::endl;
    // 取消注释下面的代码来同时订阅账户频道
    ...
    std::cout << "  - 持仓频道只推送合约和杠杆的持仓（SWAP/FUTURES/OPTION/MARGIN）" << std::endl;
    std::cout << "  - 现货（SPOT）买入后，资产在账户余额中，不在持仓频道推送" << std::endl;
    std::cout << "  - 要测试持仓频道，请开合约仓位（如BTC-USDT-SWAP永续合约）" << std::endl;
    std::cout << "  - 要查看现货余额变化，请使用账户频道（account）" << std::endl;
    std::cout << "  - 如果数据为空数组，说明当前没有合约/杠杆持仓" << std::endl;
