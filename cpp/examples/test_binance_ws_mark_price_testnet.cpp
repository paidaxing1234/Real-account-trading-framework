...
int main() {
    ...
    try {
        ...
        std::map<std::string, MarkPriceInfo> latest_prices;

        ws->set_mark_price_callback([&](const nlohmann::json& mp) {
            update_count.fetch_add(1);
            MarkPriceInfo info;
            info.symbol = mp.value("s", "");
            info.mark_price = std::stod(mp.value("p", "0"));
            info.index_price = std::stod(mp.value("i", "0"));
            info.funding_rate = std::stod(mp.value("r", "0"));
            info.next_funding_time = mp.value("T", 0LL);
            latest_prices[info.symbol] = info;
        });
        ...
        while (g_running.load()) {
            ...
            if (elapsed >= 5) {
                ...
                std::vector<std::pair<std::string, MarkPriceInfo>> sorted;
